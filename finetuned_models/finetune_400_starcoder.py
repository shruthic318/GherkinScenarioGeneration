...
def run(input_path, config):
    # Load and format your JSON data
    ...
    # Optionally save test data for inference later
    ...
    output_dir = config["starcoder"]["finetuned_400_model_dir"]
    logging_dir = config["starcoder"]["training"]["logging_dir"]
    metrics_dir = config["starcoder"]["training"]["metrics_400_dir"]
    ...
    peft_params = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=["c_proj", "c_attn", "q_proj", "v_proj", "k_proj", "out_proj", "fc_in", "fc_out"]
    )

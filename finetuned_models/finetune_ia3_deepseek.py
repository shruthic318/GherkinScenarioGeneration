...
def run(input_path, config):
   # Load requirements and gherkin scenarios(labelled data)
    ...
    output_dir = config["deepseek"]["finetuned_ia3_model_dir"]
    logging_dir = config["deepseek"]["training"]["logging_dir"]
    metrics_ia3_dir = config["deepseek"]["training"]["metrics_ia3_dir"]
    
    # Create necessary directories
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(logging_dir, exist_ok=True)
    os.makedirs(metrics_ia3_dir, exist_ok=True)
    ...
    metrics_logger = TrainingMetricsLogger(output_dir=metrics_ia3_dir)

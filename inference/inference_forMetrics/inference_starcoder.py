...
def run(input_path, config):
    ...
    # Load test data from the path specified in config
    test_data_path = config["inference"]["test_data"]
    ...
    finetuned_model_path = config["starcoder"]["finetuned_model_dir"]
    token=config["huggingface"]["token"]
    finetuned_tokenizer = AutoTokenizer.from_pretrained(finetuned_model_path,token=token)
    finetuned_model = AutoModelForCausalLM.from_pretrained(finetuned_model_path,token=token).to(device)
    ...
    base_model_path = config["starcoder"]["base_model_dir"]
    base_tokenizer = AutoTokenizer.from_pretrained(base_model_path,token=token)
    base_model = AutoModelForCausalLM.from_pretrained(base_model_path,token=token).to(device)
    base_pipe = pipeline(
        "text-generation",
        model=base_model,
    ...
    output_csv_path = "inference_results_dora_starcoder.csv"
